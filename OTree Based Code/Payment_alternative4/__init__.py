...
class Payments(Page):
    ...
    def vars_for_template(Player):
        ...
        Points = sum(ppp)
        points_total = sum(ppp)

        # points_total = sum(ppp + [Part1] + [Puzzle1])
        points_currency = round(sum(ppp ) * Constants.conversion_rate, 2)

        # points_currency = round(sum(ppp + [Part1]) * Constants.conversion_rate, 2)
        final_currency = round((sum(ppp) * Constants.conversion_rate) + 7, 2)
        ...
        return dict(total=final_currency, pointscurrency=points_currency, pointstotal=points_total,
...
page_sequence = [Questionnaire, ResultsWaitPage, Payments, Payments1, Goodbye, Check]
