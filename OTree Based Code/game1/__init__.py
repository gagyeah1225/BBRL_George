...
class Constants(BaseConstants):
    ...
    instructions_template_all = 'game1/instructions_for_all.html'
...
def set_payoffs(group):
    ...
    for p in players:
        ...
        if group.num_volunteers == 1:
            ...
            baseline_amount = Constants.general_benefit
            if (p.volunteer & (p.submission_timestamp == min(Player.submission_timestamp for Player in players))):
                p.payoff = baseline_amount - Constants.volunteer_cost
            else:
                p.payoff = baseline_amount
        else:
...
class Sent(Page):
    ...
    #form_fields = ['Helpful', 'Helpful1', 'Helpful2', 'Comfort', 'Attach', 'Familiarity']
...
# page_sequence = [Introduction_all_games,Introduction, Understanding, Understood, WaitToStart, Decision, ResultsWaitPage, Results, Finalresults]
